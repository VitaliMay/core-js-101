...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.left < rect2.left + rect2.width
    ...
    && rect2.top < rect1.top + rect1.height) return true;
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const leftBracketTrue = '[';
  const leftBracketFalse = '(';
  const rightBracketTrue = ']';
  const rightBracketFalse = ')';
  let tempA = a;
  let tempB = b;
  let result = '';
  if (a > b) {
  ...
  if (isStartIncluded) {
    result = `${leftBracketTrue}${tempA}, ${tempB}`;
  } else { result = `${leftBracketFalse}${tempA}, ${tempB}`; }

  if (isEndIncluded) {
    result += rightBracketTrue;
  } else { result += rightBracketFalse; }
...
function isCreditCardNumber(num) {
  const ccn = num.toString().split('').reverse().join('');
  const len = ccn.length;
  let result = 0;
  let tempDigit;

  for (let i = 0; i < len; i += 1) {
    tempDigit = Number(ccn[i]);
...
function isBracketsBalanced(str) {
  ...
  const result = [];
  for (let i = 0; i < str.length; i += 1) {
    if (result[result.length - 1] === bracketObj[str[i]] && result.length > 0) result.pop();
 ...
 */
function getCommonDirectoryPath(pathesArr) {
  const pathesArrMatrix = pathesArr.map((item) => item.split('/'));
  const strSlash = '/';

  let result = '';

  for (let i = 0; i < pathesArrMatrix[0].length; i += 1) {
    const currentEl = pathesArrMatrix[0][i];

    const currentElSearch = pathesArrMatrix.every((itemArr) => itemArr[i] === currentEl);

    if (currentElSearch) {
      result += currentEl + strSlash;
    } else {
      break;
    }
  }
...
function evaluateTicTacToePosition(position) {
  ...
  return undefined; // все другие случаи
}
